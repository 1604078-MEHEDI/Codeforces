...
int go(int idx, int cur)
{
  ...
  int subset = go(idx + 1, cur);
  if (cur >= bad[idx].first && cur + bad[idx].second >= 0) {
    subset = max(subset, 1 + go(idx + 1, cur + bad[idx].second));
  }
  return  dp[idx][cur] = subset;
}
...
//*/
  ...
  for (int i = 0; i < n; i++) {
    int a, b;
    cin >> a >> b;
    if (b < 0)bad.push_back({a, b});
    else good.push_back({a, b});
  }
