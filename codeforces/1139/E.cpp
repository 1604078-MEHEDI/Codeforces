...
int hopcroft_karp_max_match(int r) {
    lmt = n + r;
    for (int i = 0; i <= n + 5000; i++)match[i] = 0;
    int matching = 0;
    while (bfs()) {
        for (int i = 1; i <= n; i++)
            if (match[i] == 0 && dfs(i))
...
//*/
    ...
    for (int cs = 1; cs <= T; cs++) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (ok) {
                g[C[i]].push_back(n + P[i] + 1);
                // g[n + P[i] + 1].push_back(C[i]);
            }
        }

        for (int i = d; i >= 1; i--) {
            while (hopcroft_karp_max_match(curMex + 1) == curMex + 1)
                curMex++;
