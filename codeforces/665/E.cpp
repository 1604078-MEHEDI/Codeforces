...
inline node* Insert(node *cur, ll lvl, ll n) {
    ...
    if (b) {
        ...
        cur->right = Insert(cur->right, lvl - 1, n);
    }
    else {
        cur->lc++;
        if (cur->left == NULL)
            cur->left = new node;
        cur->left = Insert(cur->left, lvl - 1, n);
    }
    return cur;
}

inline ll Query(node *cur, ll lvl, ll n, ll k) {
    if (lvl == -1 || cur == NULL)return 0;
    ll bit_n = n & (1 << lvl);
    ll bit_k = k & (1 << lvl);
    if (bit_k) {
        if (bit_n)
            return cur->rc + Query(cur->left, lvl - 1, n, k);
        else
            return cur->lc + Query(cur->right, lvl - 1, n, k);
    }
    else {
        if (bit_n)
            return Query(cur->right, lvl - 1, n, k);
        else
            return Query(cur->left, lvl - 1, n, k);
    }
}
