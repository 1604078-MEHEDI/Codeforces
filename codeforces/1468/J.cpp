...
// bool flag = false;
  for (ll i = 0; i < edges; i++)
  {
    ...
    x = p[i].second.first;
    y = p[i].second.second;
    cost = p[i].first;//.first;
    //int ID = p[i].first.second;
  ...
  //dbg(minimumcost);
  if (minimumcost == 0)minimumcost = misir;
  return minimumcost;
}
...
void solve()
{
  ...
  initialize();
  scanf("%lld %lld %lld", &nodes, &edges, &K);
  //cout << nodes << " & "<<edges<<endl;
  for (ll i = 0; i < edges; i++)
  {
    cin >> x >> y >> weight;
    misir = min(misir, abs(K - weight));
    p[i] = make_pair(max(0ll, weight-K), make_pair(x, y));
  }
 // dbg(misir);
  sort(p, p + edges);
  // for(ll i = 0; i < edges; i++){
