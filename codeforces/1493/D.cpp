...
void add(int id, int x) {
    while (x != 1) {
        ...
        int lst = cnt_divisor[id][d];
        cnt_divisor[id][d] += add;
        int last_min = 0;
        if ((int)cnt[d].size() == n)// ei prime ta sobgulate ase kina, thakle minimum time
            last_min = (*cnt[d].begin());

        if (lst != 0)// map theke delete
            cnt[d].erase(cnt[d].find(lst));
